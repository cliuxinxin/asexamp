...
export function ProfileEditor({value,onChange}:{value:string;onChange:(value:string)=>void}){
 ...
 const samples:Json[]=Array.isArray(config.sample_cases)?config.sample_cases:[];
 ...
 <h3>项目样例</h3><p className="muted small-text">后续生成使用当前 Profile 的样例学习字段和写法。样例不作为业务需求依据；移除后需保存 Profile。</p>
 {samples.length?<>{samples.map((sample,index)=><details key={index}><summary>{index+1}. {sample.title??'用例样例'}</summary><pre>{JSON.stringify(sample,null,2)}</pre><button onClick={()=>set('sample_cases',samples.filter((_,i)=>i!==index))} aria-label={`移除样例 ${index+1}`}>移除此样例</button></details>)}<button onClick={()=>set('sample_cases',[])}>移除全部样例</button></>:<p className="empty-note">暂无样例，可在用例结果中选择“保存为项目样例”。</p>}
