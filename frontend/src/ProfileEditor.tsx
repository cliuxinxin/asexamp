...
export function ProfileEditor({value,onChange}:{value:string;onChange:(value:string)=>void}){
 ...
 const columns:Array<{field:string;header:string}>=Array.isArray(config.excel_columns)?config.excel_columns:[];
 ...
 <div className="mapping-table"><div className="mapping-row mapping-head"><span>Case 字段</span><span>Excel 列标题</span><span>顺序</span></div>{columns.map((column,index)=><div key={index} className="mapping-row"><input aria-label={`第 ${index+1} 列字段`} value={column.field} onChange={e=>update(index,'field',e.target.value)}/><input aria-label={`第 ${index+1} 列标题`} value={column.header} onChange={e=>update(index,'header',e.target.value)}/><div><button aria-label={`上移第 ${index+1} 列`} disabled={index===0} onClick={()=>move(index,-1)}>↑</button><button aria-label={`下移第 ${index+1} 列`} disabled={index===columns.length-1} onClick={()=>move(index,1)}>↓</button><button aria-label={`删除第 ${index+1} 列`} onClick={()=>set('excel_columns',columns.filter((_,i)=>i!==index))}>×</button></div></div>)}</div>
