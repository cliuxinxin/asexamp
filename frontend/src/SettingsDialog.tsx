...
export function SettingsDialog({projectId,profiles,activeProfileId,proposal,onClose,onSaved,onSelectProfile}:{projectId:string;profiles:Profile[];activeProfileId?:string;proposal?:Json;onClose:()=>void;onSaved:()=>void;onSelectProfile?:(id:string)=>void}){
 ...
 return <Dialog title="模型与设置" onClose={onClose} wide>
  ...
  {tab==='model'&&settings?<div className="settings-form">
   <p className="muted">连接本机 Ollama 或兼容的模型 API。配置保存一次，后续对话会继续使用。</p>
   {managed&&<p role="status" className="environment-notice">配置由 .env 或环境变量管理，修改后请重启服务。{settings.env_file&&<>配置文件：{settings.env_file}</>}</p>}
   <label>模型服务<select disabled={managed} value={settings.provider} onChange={e=>{const provider=e.target.value as Settings['provider'];changeEndpoint(provider,provider==='ollama'?'http://127.0.0.1:11434':'http://127.0.0.1:1234/v1');}}><option value="ollama">Ollama · 本地模型</option><option value="openai">OpenAI 兼容接口 · 本地或远程</option></select></label>
   ...
   <details className="header-settings"><summary>自定义请求头</summary>
    ...
    <label>鉴权方式<select disabled={managed} value={settings.auth_mode??'bearer'} onChange={e=>setSettings({...settings,auth_mode:e.target.value as Settings['auth_mode']})}><option value="bearer">API Key · Bearer</option><option value="headers">仅使用自定义 Header</option></select></label>
