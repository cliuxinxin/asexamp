...
export function ConversationParts({response,refreshKey,onTarget,onOpen,onChanged}:{response:TurnResponse;refreshKey?:string;onTarget:(artifact:Artifact,ids:string[])=>void;onOpen?:(artifact:Artifact)=>void;onChanged:()=>void}){
 return <div className="conversation-parts">{(response.parts??[]).map((part,index)=>{
  ...
  switch(value.type){
   ...
   case 'estimate':return <ChatEstimate key={key} estimate={value.data as any}/>;
   case 'artifact':return <ArtifactCard key={key} id={value.artifact_id} revision={value.revision} readOnly refreshKey={refreshKey} onTarget={onTarget} onOpen={onOpen} onChanged={onChanged}/>;
   case 'case_details':return <ArtifactCard key={key} id={value.artifact_id} snapshot={{id:value.artifact_id,type:'cases',title:value.title??'用例步骤与预期',revision:value.revision,items:value.items}} readOnly initialDetailsOpen onTarget={onTarget} onOpen={onOpen} onChanged={onChanged}/>;
   case 'diff':return <ConversationDiff key={key} proposalId={value.proposal_id} changes={value.changes} onChanged={onChanged}/>;
   case 'coverage':return <WorkspaceCoverage key={key} artifactId="" revision={0} initialData={value.data} initialOpen/>;
   case 'source_impact':return <SourceImpact key={key} data={value.data}/>;
...
function SourceImpact({data}:{data:Json}){
 ...
 const status=uncertain?'影响范围尚不确定，需要人工确认':broad?'检测到全局影响，需要按全部需求处理':requirements.length?'已定位可能受影响的需求':'未定位到受影响的现有需求';
...
function ConversationDiff({proposalId,changes,onChanged}:{proposalId:string;changes:Json[];onChanged:()=>void}){
 ...
 return <section className="action-preview" aria-label="变更预览">{changes.map((change,index)=><ChangePreview key={index} change={change} before={originals[change.artifact_id]}/>)}<ErrorBox message={error}/>{resolution?<p role="status">{resolution}</p>:proposalId&&<div className="actions"><button className="primary" disabled={working} onClick={()=>void resolve('artifact.apply')}>应用此修改</button><button disabled={working} onClick={()=>void resolve('artifact.discard')}>取消此修改</button></div>}</section>;
}
