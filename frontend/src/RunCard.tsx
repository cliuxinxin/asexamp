...
const stages:Record<string,string>={suspended:'恢复保存的进度',paused_edit:'修改待确认场景',routing:'理解你的请求',requirement_analysis:'分析需求',applying_clarification:'应用澄清信息',scenario_generation:'生成测试场景',case_generation:'设计测试用例',case_import:'读取已有用例',case_review:'评审与优化',publishing:'保存最终结果',resuming:'恢复任务',retrying:'重试失败阶段',route:'理解你的请求',analyze:'分析需求',analyze_requirement:'分析需求',analysis:'分析需求',clarify:'等待需求澄清',clarification:'等待需求澄清',scenarios:'生成测试场景',generate_scenarios:'生成测试场景',scenario_review:'等待场景确认',cases:'设计测试用例',generate_cases:'设计测试用例',review:'评审与优化',review_cases:'评审与优化',finalize:'保存最终结果',complete:'已完成',done:'已完成',query:'检索需求证据',learn_template:'学习模板',modify:'修改产物',queued:'等待执行'};
export function RunCard({run,onChanged,onTarget}:{run:Run;onChanged:()=>void;onTarget:(artifact:Artifact,ids:string[])=>void}){
 ...
 const detail=run.diagnostic;const waiting=!!detail&&['model.start','model.request_saved','model.waiting','model.transport_start'].includes(detail.event);
