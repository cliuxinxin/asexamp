...
export function RunCard({run,onChanged,onTarget,onPrompt,onSettings,interactionBusy=false,sources=[],onSupplementBusy,saveToProject:sharedPreference,onSaveToProjectChange,hideArtifact=false,hideActions=false,compact=false}:{run:Run;saveToProject?:boolean;onSaveToProjectChange?:(value:boolean)=>void;sources?:Source[];onSupplementBusy?:(busy:boolean)=>void;onChanged:()=>void;onTarget:(artifact:Artifact,ids:string[])=>void;onPrompt?:(text:string,artifactId?:string)=>void;onSettings?:()=>void;interactionBusy?:boolean;hideArtifact?:boolean;hideActions?:boolean;compact?:boolean}){
 ...
 return <section className={'run-card '+(compact?'compact-run ':'')+(run.status==='failed'?'failed':'')} aria-live="polite"><div className="run-heading"><span className="inline">{pending?<Spinner/>:<span className={'status-dot '+run.status}/>}<strong>{pending?(stages[run.stage]??'正在处理'):pause?(hideActions?'等待处理更新':'需要你确认'):run.status==='failed'?'任务暂停：处理失败':'任务已停止'}</strong></span><div className="actions">{run.status==='failed'&&<button disabled={working} onClick={()=>action('retry')}><RotateCcw size={15}/>重试未完成步骤</button>}{!compact&&canSupplement&&<button disabled={confirmationBusy} onClick={()=>setSupplementOpen(true)}><Plus size={14}/>补充资料</button>}{pending&&<button disabled={working} onClick={()=>action('pause')}>暂停</button>}{(pending||pause)&&<button disabled={working||supplementBusy} onClick={()=>action('cancel')}><Square size={13}/>停止</button>}</div></div>
