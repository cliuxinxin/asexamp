...
export function RunCard({chatOnly=false,run,onChanged,onTarget,onPrompt,onSettings,interactionBusy=false,sources=[],onSupplementBusy,saveToProject:sharedPreference,onSaveToProjectChange,hideArtifact=false,hideActions=false,compact=false}:{chatOnly?:boolean;run:Run;saveToProject?:boolean;onSaveToProjectChange?:(value:boolean)=>void;sources?:Source[];onSupplementBusy?:(busy:boolean)=>void;onChanged:()=>void;onTarget:(artifact:Artifact,ids:string[])=>void;onPrompt?:(text:string,artifactId?:string)=>void;onSettings?:()=>void;interactionBusy?:boolean;hideArtifact?:boolean;hideActions?:boolean;compact?:boolean}){
 ...
 return <section className={'run-card '+(compact?'compact-run ':'')+(run.status==='failed'?'failed':'')} aria-live="polite"><div className="run-heading"><span className="inline">{pending?<Spinner/>:<span className={'status-dot '+run.status}/>}<strong>{pending?(stages[run.stage]??'正在处理'):pause?(chatOnly?'等待聊天回复':hideActions?'等待处理更新':'需要你确认'):run.status==='failed'?'任务暂停：处理失败':'任务已停止'}</strong></span>{!chatOnly&&<div className="actions">{run.status==='failed'&&<button disabled={working} onClick={()=>action('retry')}><RotateCcw size={15}/>重试未完成步骤</button>}{!compact&&canSupplement&&<button disabled={confirmationBusy} onClick={()=>setSupplementOpen(true)}><Plus size={14}/>补充资料</button>}{pending&&<button disabled={working} onClick={()=>action('pause')}>暂停</button>}{(pending||pause)&&<button disabled={working||supplementBusy} onClick={()=>action('cancel')}><Square size={13}/>停止</button>}</div>}</div>
 ...
 {!compact&&run.generation_plan&&<p className="muted small-text">{run.generation_plan.reason}{run.generation_plan.input_groups>1?` · ${run.generation_plan.input_groups} 个分组`:''}</p>}
 {!compact&&run.progress&&<div className="count-progress"><div><strong>{run.progress.label}</strong><span>{run.progress.total>1?`已完成 ${run.progress.completed} / ${run.progress.total}`:run.status==='waiting'?'等待你确认':run.status==='failed'?'已暂停':'处理中'}</span></div><progress max={Math.max(1,run.progress.total)} value={run.progress.completed}/>{run.status==='failed'&&run.error&&<p className="error-text">失败：{run.error}</p>}</div>}
 {pending&&!compact&&<div className="run-progress"><p className="muted small-text">{elapsed!==undefined?`本轮已运行 ${elapsed} 秒。`:''}任务在本机后台执行，可以关闭此页面后再回来。</p>{detail?.batch_count&&<p className="small-text">第 {detail.batch_index}/{detail.batch_count} 批需求</p>}{waiting&&<p className="small-text">等待模型响应 · 本次请求 {modelElapsed} 秒{detail.timeout_seconds?` · 超时设置 ${detail.timeout_seconds} 秒`:''}{detail.attempt?` · 第 ${detail.attempt}/${detail.max_attempts} 次尝试`:''}</p>}{detail?.event==='model.retry'&&<p className="small-text">上次模型请求失败，准备第 {detail.next_attempt} 次尝试。</p>}</div>}
 ...
 {!compact&&<RunTimeline runId={run.id} restartKey={run.status} onChanged={onChanged} agentMode={agentMode} initialAgent={run.agent} initialOpen={false}/>}
 {!chatOnly&&pause&&run.interrupt?.type==='source_review'&&<div className="clarification"><p>{run.interrupt.message}</p>{run.interrupt.sources?.map(source=><label className="check-label" key={source.id}><input type="checkbox" checked={chosenSources.includes(source.id)} onChange={e=>setChosenSources(old=>e.target.checked?[...old,source.id]:old.filter(id=>id!==source.id))}/>{source.name} · 本轮作为需求使用</label>)}<textarea aria-label="补充需求正文" value={answer} onChange={e=>setAnswer(e.target.value)} placeholder="也可以直接粘贴需求正文"/><button className="primary" disabled={confirmationBusy||(!chosenSources.length&&!answer.trim())} onClick={()=>action('resume',{source_ids:chosenSources,answer})}>确认资料，继续当前任务</button></div>}
 {!chatOnly&&pause&&run.interrupt?.type==='clarification'&&<ClarificationDraftEditor runId={run.id} controlVersion={run.control_version} interruptId={run.interrupt_id} refreshKey={run.updated_at} onChanged={onChanged} disabled={confirmationBusy} saveToProject={saveToProject} onSaveToProjectChange={setSaveToProject}/>}
 {!chatOnly&&pause&&run.interrupt?.type==='workflow_paused'&&<div className="clarification"><p>{run.interrupt.message??'任务已暂停，已保存成果会保留。'}</p>{!hideActions&&<button className="primary" disabled={confirmationBusy} onClick={()=>action('resume',legacyCasesStop?{stop_after:'review'}:{},legacyCasesStop?'允许评审，继续当前任务':undefined)}>{legacyCasesStop?'允许评审，继续当前任务':'继续当前任务'}</button>}</div>}
 {!chatOnly&&pause&&gate&&run.interrupt&&<section className="scenario-review" aria-label={run.interrupt.title??gate.title}>
  ...
  {!compact&&<p className="muted small-text">查看、提问、估算和保存修改都会保留当前确认节点；应用预览后仍需明确继续。</p>}
  {!compact&&onPrompt&&<div className="actions">
  ...
  {!hideArtifact&&run.interrupt.artifact_id&&<ArtifactCard key={run.interrupt.artifact_id} id={run.interrupt.artifact_id} initialDetailsOpen={run.interrupt.type.startsWith('case_')} refreshKey={run.updated_at} onTarget={onTarget} onChanged={onChanged}/>}
  {!compact&&run.interrupt.type==='strategy_review'&&<label>本轮测试深度<select aria-label="确认测试深度" value={confirmedDepth} disabled={confirmationBusy} onChange={e=>setConfirmedDepth(e.target.value)}><option value="">沿用本轮 Profile</option><option value="quick">Quick · 核心路径</option><option value="standard">Standard · 主要规则</option><option value="deep">Deep · 深入组合</option></select></label>}
  {confirmationBusy&&<p className="small-text">{confirmationMessage}</p>}
  {!hideActions&&<button className="primary" disabled={confirmationBusy} onClick={()=>action('resume',{approved:true,...(run.interrupt?.type==='strategy_review'&&confirmedDepth?{depth:confirmedDepth}:{}),...(goalReached?{stop_after:gate.next}:{})},confirmLabel)}><Check size={16}/>{confirmLabel}</button>}
 </section>}
 {!chatOnly&&supplementOpen&&<ProjectSupplement run={run} sources={sources} onClose={()=>setSupplementOpen(false)} onChanged={onChanged} onBusy={supplementWorking} disabled={interactionBusy||!!run.edit_in_progress}/>}
