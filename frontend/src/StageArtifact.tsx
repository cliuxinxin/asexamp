...
export function StageArtifact({id,revision,stage,onOpenLatest,onOpenSnapshot}:{id:string;revision:number;stage:string;onOpenLatest?:(artifact:Artifact)=>void;onOpenSnapshot?:(artifact:Artifact)=>void}){
 ...
 return <details className="stage-artifact" onToggle={e=>{if(e.currentTarget.open)load();}}><summary>查看{title} · v{revision}</summary>{loading&&<p><Spinner/>正在读取已保存的阶段成果…</p>}<ErrorBox message={error}/>{error&&<button onClick={load}>重新读取</button>}{data&&<><p className="muted small-text">这是当前阶段保存的版本，历史内容保持不变。</p>{workspace.open||onOpenSnapshot?<button onClick={()=>workspace.open?workspace.open({artifactId:id,revision,readOnly:true}):onOpenSnapshot?.({...data,id,revision})}>展开此版本成果</button>:onOpenLatest&&<button disabled={loading} onClick={openLatest}>打开最新版本并编辑</button>}{data.report&&<AnalysisReport report={data.report}/>}</>}</details>;
}
