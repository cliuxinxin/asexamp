...
export function ChangePreview({change,before,compact=false}:{change:Json;before?:Artifact;compact?:boolean}){
 const [visible,setVisible]=useState(compact?2:25);const [expandedFields,setExpandedFields]=useState<Record<string,boolean>>({});
 ...
 return <section className="artifact-change"><h4>{change.title??before?.title??change.artifact_id} · v{change.expected_revision} → 新版本</h4><p className="muted small-text">新增 {added} · 更新 {altered.length-added-removed} · 删除 {removed}</p>{altered.slice(0,visible).map(id=>{
  const old=prior.get(id),item=next.get(id);const fields=[...new Set([...Object.keys(old??{}),...Object.keys(item??{})])].filter(key=>!key.startsWith('_')&&!equalConfig(old?.[key],item?.[key]));const shownFields=compact&&!expandedFields[id]?fields.slice(0,4):fields;const operation=(change.operations??[]).find((op:Json)=>op.id===id&&op.op==='delete');
  return <div className="item-change" key={id}><strong>{id} · {item?.title??old?.title} · {!old?'新增':!item?'删除':'更新'}</strong>{operation?.reason&&<p className="action-deletion-reason">删除原因：{operation.reason}</p>}<div className="table-scroll"><table aria-label={id+' 修改对比'} className="artifact-change-table"><thead><tr><th scope="col">字段</th><th scope="col">修改前</th><th scope="col">修改后</th></tr></thead><tbody>{shownFields.map(field=><tr key={field}><th scope="row">{labels[field]??field}</th><td><div className="artifact-diff-cell"><ChangedValue field={field} before={old?.[field]} after={item?.[field]} side="before"/></div></td><td><div className="artifact-diff-cell"><ChangedValue field={field} before={old?.[field]} after={item?.[field]} side="after"/></div></td></tr>)}</tbody></table></div>{shownFields.length<fields.length&&<button className="inline-fields-more" onClick={()=>setExpandedFields(old=>({...old,[id]:true}))}>展开其余 {fields.length-shownFields.length} 个字段</button>}</div>;
 })}{altered.length>visible&&<button onClick={()=>setVisible(old=>old+(compact?5:25))}>再查看 {Math.min(compact?5:25,altered.length-visible)} 条修改</button>}{!altered.length&&<p className="muted">条目内容没有变化。</p>}{change.report&&!equalConfig(change.report,before?.report??change.before_report)&&<details><summary>查看报告更改</summary><div className="action-report-diff"><div><strong>修改前</strong><TextValue value={before?.report??change.before_report??{}}/></div><div><strong>修改后</strong><TextValue value={change.report}/></div></div></details>}</section>;
}
