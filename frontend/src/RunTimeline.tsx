...
export function RunTimeline({runId,restartKey='',onChanged,initialOpen=true,keepAlive=false}:{runId:string;restartKey?:string;onChanged?:()=>void;initialOpen?:boolean;keepAlive?:boolean}){
 ...
 const connected=keepAlive||open;
 ...
 useEffect(()=>{
  ...
  const flush=()=>{timer=undefined;const batch=pending;pending=[];if(active&&batch.length)setFeed(old=>batch.reduce((state,e)=>reduceEvent(state,e.kind,e.id,e.data),old));};
  ...
  const receive=(kind:string)=>(raw:Event)=>{
   ...
   try{const data=JSON.parse(event.data);if(data.run_id&&data.run_id!==runId)return;
    ...
    if(kind==='update')refresh();
   }catch{setConnection('invalid');source.close();}
  };
  for(const kind of ['update','progress','model_delta'])source.addEventListener(kind,receive(kind));
