...
import type {AgentState,Artifact,Coverage} from './types';
...
export function AgentPanel({agent,runId,runStatus='running',onTarget,onChanged}:{agent?:AgentState;runId?:string;runStatus?:string;onTarget?:(artifact:Artifact,ids:string[])=>void;onChanged?:()=>void}){
 if(!agent)return <p className="agent-intro muted">{runStatus==='running'?'正在读取资料，准备本次工作…':'本次没有已保存的工作摘要。'}</p>;
 ...
 return <div className="agent-panel" aria-label="测试设计进展">
  ...
  {runId&&(agent.work||agent.preview_ids?.length)&&<WorkPanel key={runId} runId={runId} runStatus={runStatus} work={agent.work} previewIds={agent.preview_ids} onTarget={onTarget} onChanged={onChanged}/>}
  {!agent.work&&plan.length>0&&<div className="agent-plan"><div className="agent-section-label"><ListChecks size={16}/><strong>本次计划</strong><small>{plan.filter(step=>step.status==='completed').length} / {plan.length} 项完成</small></div><ol>{plan.map(step=><li key={step.id} className={'plan-step '+step.status}>{step.status==='completed'?<Check size={15}/>:step.status==='running'&&runStatus==='running'?<Spinner/>:<Circle size={13}/>}<span>{step.title}</span>{step.status==='blocked'&&<small>等待确认</small>}</li>)}</ol></div>}
