...
export function App(){
 ...
 const [optionsOpen,setOptionsOpen]=useState(false);const [depth,setDepth]=useState<Depth>('auto');const [confirmStrategy,setConfirmStrategy]=useState(true);
 ...
 const [chatId,setChatId]=useState('');const [snapshot,setSnapshot]=useState<Snapshot>();const [intent,setIntent]=useState('auto');const [mode]=useState('auto');
 ...
 const latestRun=snapshot?.runs?.slice().sort((a,b)=>b.updated_at.localeCompare(a.updated_at)).find(r=>busy(r));const running=busy(latestRun);const canInstruct=!!(running&&latestRun?.experience==='agent');const canEditPaused=!!(target&&latestRun?.status==='waiting'&&latestRun.interrupt?.type==='scenario_review'&&target.artifact.id===latestRun.interrupt.artifact_id);const refreshKey=snapshot?.runs?.map(r=>r.id+':'+r.updated_at+':'+r.status).join('|')+':'+artifactEpoch;
 ...
 async function send(){
  ...
  try{
   ...
   else await api('/chats/'+id+'/messages',{content:message,experience:'agent',depth,confirm_strategy:confirmStrategy,as_requirement:asRequirement,intent:submittedTarget?'modify':intent,mode,profile_id:profileId||undefined,...(submittedTarget?{artifact_id:submittedTarget.artifact.id,...(submittedTarget.ids.length?{selected_ids:submittedTarget.ids}:{})}:{})});
