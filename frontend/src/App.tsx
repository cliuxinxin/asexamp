...
export function App(){
 ...
 const [optionsOpen,setOptionsOpen]=useState(false);const [depth,setDepth]=useState<Depth>('auto');const [confirmStrategy,setConfirmStrategy]=useState(false);
 ...
 const latestRun=snapshot?.runs?.slice().sort((a,b)=>b.updated_at.localeCompare(a.updated_at)).find(r=>busy(r));const running=busy(latestRun);const canInstruct=!!(running&&(latestRun?.experience==='agent'||(latestRun?.graph_version??0)>=2));const canEditPaused=!!(target&&latestRun?.status==='waiting'&&latestRun.interrupt?.type==='scenario_review'&&target.artifact.id===latestRun.interrupt.artifact_id);const refreshKey=snapshot?.runs?.map(r=>r.id+':'+r.updated_at+':'+r.status).join('|')+':'+artifactEpoch;
 ...
 async function send(){
  if(!content.trim()||projectLoading||sending||uploading||(running&&!canEditPaused&&!canInstruct))return;
  ...
  try{
   ...
   else await api('/chats/'+id+'/messages',{content:message,experience:'agent',depth,confirm_strategy:confirmStrategy,as_requirement:asRequirement,intent:submittedTarget?'modify':intent,mode:confirmStrategy?'hitp':'auto',profile_id:profileId||undefined,...(submittedTarget?{artifact_id:submittedTarget.artifact.id,...(submittedTarget.ids.length?{selected_ids:submittedTarget.ids}:{})}:{})});
 ...
 function selectTarget(artifact:Artifact,ids:string[]){if(running&&!(latestRun?.status==='waiting'&&latestRun.interrupt?.type==='scenario_review'&&artifact.id===latestRun.interrupt.artifact_id)){setError('请在当前任务完成后修改已有结果。本轮补充要求可直接在聊天中发送。');return;}setTarget({artifact,ids});textarea.current?.focus();}
 ...
 return <div className="app-shell">
  ...
  <div className="composer-wrap"><div className="composer-inner"><ErrorBox message={error}/>{target&&<div className="target-bar"><span>修改：{target.artifact.title}{target.ids.length?` · 选中 ${target.ids.length} 条`:' · 全部条目'}</span><button className="icon-button" aria-label="取消修改目标" onClick={()=>setTarget(undefined)}><X size={15}/></button></div>}
   ...
   <div className="composer"><div className="composer-input"><textarea ref={textarea} aria-label="聊天输入" rows={2} placeholder={canInstruct?'可以补充要求，我会调整当前任务…':target?'告诉我如何修改这些条目…':'描述你想测试的内容，或上传需求文档…'} value={content} onChange={e=>setContent(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'&&!e.shiftKey&&!e.nativeEvent.isComposing){e.preventDefault();send();}}}/><button className="send-button" aria-label="发送消息" disabled={!content.trim()||projectLoading||sending||(running&&!canEditPaused&&!canInstruct)||uploading} onClick={send}>{sending?<Spinner/>:<ArrowUp size={23}/>}</button></div><div className="composer-tools"><div className="tool-group"><input ref={fileInput} type="file" multiple accept=".docx,.pdf,.xlsx,.xls,.csv,.txt,.md" hidden onChange={e=>upload(e.target.files)}/><button className="icon-button attachment-button" disabled={projectLoading||uploading||running} title="上传需求文档" aria-label="上传需求文档" onClick={()=>fileInput.current?.click()}>{uploading?<Spinner/>:<Paperclip size={21}/>}</button><button className="task-options-toggle" aria-expanded={optionsOpen} onClick={()=>setOptionsOpen(value=>!value)}>本次方案 · {depthNames[depth]}<ChevronDown size={14}/></button>{canInstruct&&<span className="composer-live">可随时补充要求</span>}</div></div>
   {optionsOpen&&<div className="task-options"><label>测试设计深度<select value={depth} onChange={e=>setDepth(e.target.value as Depth)}>{Object.entries(depthNames).map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></label><p className="option-description">{depth==='quick'?'聚焦核心路径与重要风险，列明未展开的范围。':depth==='deep'?'深入状态变化、条件组合和跨模块风险。':depth==='standard'?'覆盖日常需求的业务规则、异常和边界。':'结合业务风险、状态与依赖推荐设计深度。'}</p><label className="check-label"><input type="checkbox" checked={confirmStrategy} onChange={e=>setConfirmStrategy(e.target.checked)}/>生成用例前，先与我确认业务图和测试方向</label><details><summary>指定任务与资料用途</summary><div className="advanced-task-options"><label>任务目标<select aria-label="任务目标" disabled={!!target} value={intent} onChange={e=>setIntent(e.target.value)}>{intents.map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></label><label>上传文件角色<select aria-label="上传文件角色" value={uploadRole} onChange={e=>setUploadRole(e.target.value)}>{Object.entries(roles).map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></label><label className="check-label"><input type="checkbox" aria-label="这条消息是需求正文" checked={asRequirement} onChange={e=>setAsRequirement(e.target.checked)}/>将这条消息保存为需求正文</label></div></details></div>}
   </div><p className="composer-note">{canInstruct?'补充内容会在安全的步骤边界应用，已有结果会保留。':running?'任务执行中，停止或完成后可发送下一条消息。':'Enter 发送 · Shift + Enter 换行'}<span>结果可追溯至需求原文，请结合业务复核。</span></p>
  </div></div></main>
  {openedArtifact&&<ArtifactWorkspace key={openedArtifact.id} artifact={openedArtifact} refreshKey={refreshKey} onClose={()=>setOpenedArtifact(undefined)} onTarget={selectTarget} onChanged={()=>{setArtifactEpoch(x=>x+1);refresh();}}/>}
  {settingsOpen&&<SettingsDialog projectId={projectId} profiles={profiles} activeProfileId={profileId} onSelectProfile={setProfileId} proposal={proposal} onClose={()=>setSettingsOpen(false)} onSaved={()=>reloadSettings().catch(e=>setError(errText(e)))}/>}
