...
export function App(){
 ...
 const fillComposer=(text:string)=>{const previous=draftContent.current;const next=previous?previous+'\n'+text:text;draftContent.current=next;setContent(next);textarea.current?.focus();};
 ...
 return <ConversationContext.Provider value={{chatId,onChanged:changed}}><ArtifactWorkspaceContext.Provider value={{open:openArtifactWorkspace}}><div inert={!!artifactWorkspace} className={'app-shell v28 v29'+(sidebarCollapsed?' sidebar-collapsed':'')+(messages.length?' has-messages':' intake-page')}>
  ...
  <div className="workspace-layout">
   ...
   {(displayedPrompt?.kind!=='case_result_review'||!activeProposalId)&&<ConversationPrompt errorInConversation={messages.some(message=>message.metadata?.pipeline_failure&&message.metadata?.run_id===displayedPrompt?.run_id)} prompt={displayedPrompt} waiting={interactionDisabled} onFill={fillComposer} onAnswer={(id,question,answer)=>displayedPrompt?send(undefined,undefined,undefined,{text:clarificationReplyText(id,question,answer),kind:'clarification',promptId:displayedPrompt.id,command:{name:'clarification.answer',arguments:{answers:{[id]:answer}}}}):false}/>}
   ...
   <ConversationSuggestions key={chatId+':'+(displayedPrompt?.id??'')} prompt={displayedPrompt?.kind==='case_result_review'&&activeProposalId?undefined:displayedPrompt} disabled={sending||projectLoading||uploading||operationBusy||!!latestRun?.edit_in_progress||latestRun?.status==='queued'||latestRun?.status==='running'} onFill={fillComposer} onViewProfileChange={()=>{if(displayedPrompt?.kind==='profile'&&chatId)setProfilePreview({chatId,promptId:displayedPrompt.id});}} onChoose={(text,kind)=>{if(displayedPrompt)void send(undefined,undefined,undefined,{text,kind,promptId:displayedPrompt.id});}}/>
