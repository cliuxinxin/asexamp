...
import type {Artifact,Chat,ConversationPrompt as Prompt,Depth,Json,Profile,Project,Settings,Snapshot,TurnRequest} from './types';
...
export function App(){
 ...
 useEffect(()=>{if(viewer)return;if(nearBottom){bottom.current?.scrollIntoView({behavior:'smooth',block:'end'});setNewActivity(false);}else setNewActivity(true);},[chatId,snapshot?.messages.length,latestRun?.updated_at,displayedPrompt?.id]);
 ...
 async function send(generationMessage?:string, requestedIntent?:string, generationArgs?:Json){
  ...
  try{
   ...
   const explicitInitialTask=!submittedPrompt&&!generationMessage&&!artifact&&!submittedTarget&&!sending&&!Array.from(pendingSends.current.values()).some(send=>send.inFlight)&&snapshot?.chat.id===id&&snapshot.runs.length===0&&['review_requirement','generate_scenario','generate_case'].includes(taskIntent);
   const body:Omit<TurnRequest,'client_message_id'>={content:message,...(submittedPrompt?{reply_to:submittedPrompt.id}:{}),intent_hint:taskIntent,mode:displayedMode,profile_id:profileId||undefined,...options,...(artifact?{artifact_id:artifact.id,artifact_revision:artifact.revision}:{}),...(submittedTarget?.viewOrder?{view_order:submittedTarget.viewOrder}:{}),...(submittedTarget?.ids.length?{selected_ids:submittedTarget.ids}:{}),...((generationMessage||explicitInitialTask)?{command:{name:readonlyCaseReview?'artifact.review_cases':'workflow.start',arguments:readonlyCaseReview?{instruction:message}:{intent:taskIntent==='auto'?'generate_case':taskIntent,depth,...(caseTypesTouched?{case_types:caseTypes}:{}),...(runOverride?{profile_override:runOverride}:{}),as_requirement:generationArgs?.as_requirement??asRequirement}}}:{})};
   ...
   setRunOverride(undefined);if(!generationMessage)clearSubmitted(pid+':'+id,submittedVersion,true);
 ...
 return <ConversationContext.Provider value={{chatId,onChanged:changed}}><div className={'app-shell v28 v29'+(sidebarCollapsed?' sidebar-collapsed':'')+(snapshot?.messages.length?' has-messages':' intake-page')}>
