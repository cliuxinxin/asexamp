...
import type {ProcessArtifact} from './ProcessArtifactPicker';
import {InlineChangeCard} from './InlineChangeCard';
import {TraceabilityPanel} from './TraceabilityPanel';
import {FullScreenReviewer} from './FullScreenReviewer';
...
export function App(){
 const [tableReview,setTableReview]=useState<TableReviewRequest>();
 ...
 const [viewerError,setViewerError]=useState('');const [viewerLoading,setViewerLoading]=useState(false);
 ...
 useEffect(()=>{setProfilePreview(undefined);setMemoryOpen(false);setTraceabilityOpen(false);setTableReview(undefined);},[chatId,projectId]);
 ...
 useEffect(()=>{if(!chatId)return;let alive=true;api<Json>('/chats/'+chatId+'/workspace-state').then(value=>{if(alive)setWorkspaceProposal(value.pending_proposal);}).catch(()=>{if(alive)setWorkspaceProposal(undefined);});return()=>{alive=false;};},[chatId,refreshKey,displayedPrompt?.id]);
 ...
 useEffect(()=>{if(viewer||tableReview)return;if(nearBottom){bottom.current?.scrollIntoView({behavior:'smooth',block:'end'});setNewActivity(false);}else setNewActivity(true);},[chatId,messages.length,latestRun?.updated_at,displayedPrompt?.id]);
 ...
 const openResult=(a:Artifact)=>{viewerRequest.current++;setViewerError('');setViewerLoading(false);viewerScroll.current=conversationScroll.current?.scrollTop??0;viewerFocus.current=document.activeElement as HTMLElement;setViewer({artifact:a,historical:true});};
 ...
 const openCurrentResult=async()=>{if(!visibleOutput)return;setViewerError('');setViewerLoading(false);const request=++viewerRequest.current,currentChat=chatId;viewerScroll.current=conversationScroll.current?.scrollTop??0;viewerFocus.current=document.activeElement as HTMLElement;try{const artifact=await api<Artifact>('/artifacts/'+encodeURIComponent(visibleOutput));if(request===viewerRequest.current&&chatRef.current===currentChat)setViewer({artifact,historical:false});}catch(e){if(request===viewerRequest.current&&chatRef.current===currentChat)setError(errText(e));}};
 const openProcessResult=async(entry:ProcessArtifact)=>{const request=++viewerRequest.current,currentChat=chatId,token=nav.current;setViewerError('');setViewerLoading(true);try{const latest=await api<Artifact>('/artifacts/'+encodeURIComponent(entry.artifact_id));const historical=latest.revision!==entry.revision;const artifact=historical?await api<Artifact>('/artifacts/'+encodeURIComponent(entry.artifact_id)+'/revisions/'+entry.revision):latest;if(request===viewerRequest.current&&chatRef.current===currentChat&&nav.current===token)setViewer({artifact,historical});}catch(e){if(request===viewerRequest.current&&chatRef.current===currentChat&&nav.current===token)setViewerError(errText(e));}finally{if(request===viewerRequest.current)setViewerLoading(false);}};
 ...
 const activePreviewMessageId=activeProposalId?messages.filter(message=>message.role==='assistant'&&(message.metadata?.review_proposal_id===activeProposalId||(message.metadata?.turn_response?.parts??[]).some((part:Json)=>part.type==='diff'&&part.proposal_id===activeProposalId))).at(-1)?.id:undefined;
 ...
 const resolveInline=async()=>{setWorkspaceProposal(undefined);await refresh();changed();};
 ...
 useEffect(()=>{if(!traceTarget||chatId!==traceTarget.chatId||snapshot?.chat.id!==traceTarget.chatId)return;let active=true;const current=traceTarget;void api<Artifact>('/artifacts/'+encodeURIComponent(current.artifactId)).then(artifact=>{if(!active||chatRef.current!==current.chatId)return;if(artifact.chat_id&&artifact.chat_id!==current.chatId)throw new Error('成果不属于当前对话');viewerScroll.current=conversationScroll.current?.scrollTop??0;viewerFocus.current=document.activeElement as HTMLElement;setViewer({artifact:{...artifact,items:artifact.items.filter(row=>String(row.id)===current.itemId),view_item_ids:[current.itemId]},historical:false});setTraceTarget(undefined);}).catch(e=>{if(active){setError(errText(e));setTraceTarget(undefined);}});return()=>{active=false;};},[traceTarget,chatId,snapshot?.chat.id]);

 const openTableReview=(request:TableReviewRequest)=>{
  if(!viewer){viewerScroll.current=conversationScroll.current?.scrollTop??0;viewerFocus.current=document.activeElement as HTMLElement;}
  viewerRequest.current++;setViewer(undefined);setTableReview(request);
 };
 const closeTableReview=()=>{setTableReview(undefined);const restore=()=>{if(conversationScroll.current)conversationScroll.current.scrollTop=viewerScroll.current;viewerFocus.current?.focus();};if(typeof window.requestAnimationFrame==='function')window.requestAnimationFrame(restore);else setTimeout(restore,0);};
 const sendTableMessage=async(text:string,ids:string[],binding?:{artifactRevision:number;proposalId?:string;promptId?:string})=>{
  if(!tableReview||tableReview.readOnly)throw new Error('请打开当前版本后再修改。');
  if(sending||operationBusy||latestRun?.status==='running'||latestRun?.status==='queued')throw new Error('当前操作仍在处理中，请稍候再发送。');
  const currentChat=chatRef.current;
  const artifact=await api<Artifact>('/artifacts/'+encodeURIComponent(tableReview.artifactId));
 ...
 return <ConversationContext.Provider value={{chatId,onChanged:changed}}><TableReviewContext.Provider value={{open:openTableReview}}><div inert={!!tableReview} className={'app-shell v28 v29'+(sidebarCollapsed?' sidebar-collapsed':'')+(messages.length?' has-messages':' intake-page')}>
  ...
  {viewer&&<Dialog title={(viewer.historical?'历史成果':'查看成果')+' · '+viewer.artifact.title} onClose={closeViewer} wide><ProcessArtifactPicker chatId={chatId} artifact={viewer.artifact} refreshKey={refreshKey} onSelect={entry=>void openProcessResult(entry)}/><ErrorBox message={viewerError}/>{viewerLoading&&<p role="status"><Spinner/>正在读取所选成果…</p>}<div className="result-viewer-label"><strong>{viewer.historical?'历史版本 · 只读':'当前成果'}</strong><span>v{viewer.artifact.revision}</span>{viewer.historical&&<button onClick={()=>void openLatestFromHistory(viewer.artifact.id)}>查看最新版本</button>}</div><ArtifactCard key={viewer.artifact.id+':'+viewer.artifact.revision} chatOnly onProfileProposal={promptId=>openFieldProposal(chatId,promptId)} id={viewer.artifact.id} snapshot={viewer.artifact} initialSelectedIds={!viewer.historical&&target?.artifact.id===viewer.artifact.id?target.ids:[]} readOnly={viewer.historical} simplified={false} refreshKey={refreshKey} onSelectionChange={viewer.historical?undefined:trackSelection} onTarget={(artifact,ids)=>{selectTarget(artifact,ids);closeViewer();}} onChanged={changed}/><div className="dialog-actions viewer-actions"><button onClick={closeViewer}>返回对话</button>{!viewer.historical&&<button className="primary" disabled={target?.artifact.id!==viewer.artifact.id||!target.ids.length} onClick={()=>{const selected=target?.artifact.id===viewer.artifact.id?target.ids:[];selectTarget(viewer.artifact,selected,viewedTable?.viewOrder);if(!draftContent.current.trim())setContent('请修改所选条目：');closeViewer();}}>在对话中修改所选</button>}</div></Dialog>}
 ...
 </div>{tableReview&&<FullScreenReviewer request={tableReview} chatId={chatId} messages={messages} refreshKey={refreshKey+':'+(displayedPrompt?.id??'')} onClose={closeTableReview} onSaved={async()=>{await refresh();changed();closeTableReview();}} onSend={sendTableMessage}/>}</TableReviewContext.Provider></ConversationContext.Provider>;
}
