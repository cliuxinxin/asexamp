...
import {ArtifactChangeDialog} from './ArtifactChangeDialog';
...
export function App(){
 const [artifactPreview,setArtifactPreview]=useState<{chatId:string;prompt:Prompt;proposalId:string}>();
 ...
 useEffect(()=>{setProfilePreview(undefined);setArtifactPreview(undefined);setMemoryOpen(false);},[chatId,projectId]);
 ...
 return <ConversationContext.Provider value={{chatId,onChanged:changed}}><div className={'app-shell v28 v29'+(sidebarCollapsed?' sidebar-collapsed':'')+(messages.length?' has-messages':' intake-page')}>
  ...
  <div className="workspace-layout">
   ...
   <ConversationSuggestions key={chatId+':'+(displayedPrompt?.id??'')} prompt={displayedPrompt} hasArtifactPreview={(displayedPrompt?.kind==='artifact_proposal'&&!!workspaceProposal)||!!displayedPrompt?.review_proposal_id} onViewArtifactChange={()=>{const proposalId=displayedPrompt?.review_proposal_id??workspaceProposal?.id;if(chatId&&displayedPrompt&&proposalId)setArtifactPreview({chatId,prompt:displayedPrompt,proposalId});}} disabled={sending||projectLoading||uploading||operationBusy||!!latestRun?.edit_in_progress||latestRun?.status==='queued'||latestRun?.status==='running'} onViewProfileChange={()=>{if(displayedPrompt?.kind==='profile'&&chatId)setProfilePreview({chatId,promptId:displayedPrompt.id});}} onChoose={(text,kind)=>{if(displayedPrompt)void send(undefined,undefined,undefined,{text,kind,promptId:displayedPrompt.id});}}/>
  ...
  {artifactPreview&&<ArtifactChangeDialog key={artifactPreview.chatId+':'+artifactPreview.proposalId} {...artifactPreview} onClose={()=>setArtifactPreview(undefined)} onRevise={()=>{setArtifactPreview(undefined);textarea.current?.focus();}} onResolved={async()=>{setArtifactPreview(undefined);setWorkspaceProposal(undefined);await refresh();changed();}}/>}
