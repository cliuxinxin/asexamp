...
export function AnalysisReport({report}:{report:Json}){
 ...
 return <div className="analysis-report">
  ...
  {Array.isArray(report.review_reports)&&report.review_reports.map((review:Json,index:number)=><div className="report-findings" key={'review-'+index}><strong>AI 审核结论</strong><p>{describe(review)}</p>{review.changes&&<p>新增 {review.changes.add??0} 条 · 修改 {review.changes.update??0} 条 · 删除 {review.changes.delete??0} 条</p>}</div>)}
