...
function describe(value:unknown):string{
 ...
 if(value&&typeof value==='object'){const item=value as Json;const text=item.summary??item.title??item.description??item.question??value;return typeof text==='string'?text:JSON.stringify(text);}
 return String(value??'');
}
...
export function AnalysisReport({report}:{report:Json}){
 ...
 const groups=[['questions','需要确认'],['clarification_followups','补充问题（待核实，尚未确认为业务规则）'],['assumptions','尚未确认的假设'],['issues','检查发现'],['limitations','范围限制'],['coverage','覆盖说明'],['in_scope','测试范围'],['out_of_scope','范围排除项']] as const;
 return <div className="analysis-report">
  {(report.clarification||report.clarification_note)&&<div className="report-findings"><strong>澄清说明</strong><p className="preserve">{describe(report.clarification)}</p><small>{describe(report.clarification_note)}</small></div>}
  ...
  {Array.isArray(report.scenario_exclusions)&&report.scenario_exclusions.length>0&&<div className="report-findings"><strong>评审确认的范围排除项</strong><ul>{report.scenario_exclusions.map((e:Json,i:number)=><li key={i}>{e.scenario_id}：{e.reason}<EvidenceRefs refs={e.refs??[]}/></li>)}</ul></div>}
  {Array.isArray(report.review_reports)&&report.review_reports.map((review:Json,index:number)=><div className="report-findings" key={'review-'+index}><strong>AI 审核结论</strong>{review.changes&&<p>新增 {review.changes.add??0} 条 · 修改 {review.changes.update??0} 条 · 删除 {review.changes.delete??0} 条</p>}<AnalysisReport report={{...review,review_reports:undefined}}/></div>)}
