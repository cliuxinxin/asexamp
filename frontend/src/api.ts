...
export async function downloadArtifact(id:string,layout:string|undefined,selected:string[],profileId?:string,revision?:number){
 const params=new URLSearchParams();if(layout)params.set('layout',layout);if(selected.length)params.set('ids',selected.join(','));if(profileId)params.set('profile_id',profileId);if(revision!==undefined)params.set('revision',String(revision));
