...
import {applyDecisions,canonicalField,cellChanges,cloneItems,EMPTY_COLUMN_CHANGES,globalIssues,isCustomColumn,issuesForCell,issueText,newWorkspaceItem,reviewChanges,rowKey} from './table-review-model';
import type {ColumnChanges,ReviewChange,ReviewColumn,ReviewDecision,ReviewRow,ArtifactWorkspaceData} from './table-review-model';

type SendBinding={artifactRevision:number;proposalId?:string;promptId?:string};
type Props={request:ArtifactWorkspaceRequest;chatId:string;messages:Json[];refreshKey?:string|number;onClose:()=>void;onSaved:(result:Json)=>void|Promise<void>;onSend:(text:string,selectedIds:string[],binding?:SendBinding)=>Promise<void>};
...
const signature=(data:ArtifactWorkspaceData)=>JSON.stringify([data.artifact_revision,data.proposal_id,data.profile_revision,data.artifact_type,data.mode,data.columns,data.proposed_items,data.report,data.read_only,data.prompt_id]);
...
export function ArtifactWorkspace({request,chatId,messages,refreshKey,onClose,onSaved,onSend}:Props){
 const [data,setData]=useState<ArtifactWorkspaceData|null>(null),[draft,setDraft]=useState<Json[]>([]),[rows,setRows]=useState<ReviewRow[]>(emptyRows);
 const [decisions,setDecisions]=useState<Record<string,ReviewDecision>>({}),[manual,setManual]=useState(false);
 const [layout,setLayout]=useState('case'),[filter,setFilter]=useState<Filter>('all'),[selected,setSelected]=useState<string[]>([]);
 ...
 const [incoming,setIncoming]=useState<ArtifactWorkspaceData|null>(null),[editor,setEditor]=useState<Editor|null>(null),[activeCell,setActiveCell]=useState<string|null>(null),[exitWarning,setExitWarning]=useState(false),[visible,setVisible]=useState(100);
 const [reportDraft,setReportDraft]=useState<Json>({}),[columnChanges,setColumnChanges]=useState<ColumnChanges>(EMPTY_COLUMN_CHANGES),[columnEditor,setColumnEditor]=useState(false),[columnField,setColumnField]=useState(''),[columnHeader,setColumnHeader]=useState('');
 const container=useRef<HTMLDivElement>(null),loadSerial=useRef(0),projectionSerial=useRef(0),latestData=useRef<ArtifactWorkspaceData|null>(null),latestDirty=useRef(false),saving=useRef(false),chatSending=useRef(false),saveReceipt=useRef<{fingerprint:string;id:string}|null>(null),chatLog=useRef<HTMLDivElement>(null),baselineColumns=useRef<ReviewColumn[]>([]),baselineColumnChanges=useRef<ColumnChanges>(EMPTY_COLUMN_CHANGES);
 ...
 const readOnly=!!request.readOnly||data?.mode==='read_only'||!!data?.read_only;
 const proposalMode=data?.mode==='ai_proposal';
 const locked=readOnly||!!incoming||busy||loading||chatBusy;
 const endpoint=`/artifacts/${encodeURIComponent(request.artifactId)}/workspace-grid`;
 const body=useCallback((items:Json[],mode=layout,baseline=false)=>{const changes=baseline?baselineColumnChanges.current:columnChanges;return {items,layout:mode,expected_revision:data?.artifact_revision,profile_id:data?.profile_id,profile_revision:data?.profile_revision,run_id:data?.run_id,proposal_id:data?.proposal_id,prompt_id:data?.prompt_id,revision:readOnly?data?.artifact_revision:undefined,...(!baseline&&data&&!equalConfig(reportDraft,data.proposed_report??data.report??{})?{report:reportDraft}:{}),...(changes.added.length||changes.removed.length?{column_changes:changes}:{})};},[data,layout,readOnly,reportDraft,columnChanges]);
 const install=useCallback((next:ArtifactWorkspaceData)=>{baselineColumns.current=structuredClone(next.columns);baselineColumnChanges.current=structuredClone(next.column_changes??EMPTY_COLUMN_CHANGES);setData(next);setDraft(cloneItems(next.proposed_items));setRows(next.proposed_rows);setLayout(next.layout);setReportDraft(structuredClone(next.proposed_report??next.report??{}));setColumnChanges(next.column_changes??EMPTY_COLUMN_CHANGES);setDecisions({});setManual(false);setIncoming(null);setEditor(null);setActiveCell(null);setError('');setProjectionError(false);setNotice('');setVisible(100);
 ...
 const load=useCallback(async(discard=false,discover=false)=>{
  ...
  try{
   const next=await api<ArtifactWorkspaceData>(endpoint+'?'+params);
 ...
 useEffect(()=>{
  ...
  const timer=setTimeout(()=>{
   void api<{columns:ReviewColumn[];rows:ReviewRow[]}>(endpoint+'/project',body(draft)).then(next=>{
    ...
    setRows(next.rows);if(!latestDirty.current)baselineColumns.current=structuredClone(next.columns);setData(old=>old?{...old,columns:next.columns}:old);setProjecting(false);
 ...
 useEffect(()=>{
  ...
  let live=true;
  void Promise.all([api<{rows:ReviewRow[]}>(endpoint+'/project',body(data.original_items,layout,true)),api<{rows:ReviewRow[]}>(endpoint+'/project',body(data.proposed_items,layout,true))]).then(([original,proposed])=>{
 ...
 const gridRows=useMemo(()=>{
  ...
  const visibleRows=proposalMode?[...(data?.proposed_rows??[]),...(data?.original_rows??[]),...rows]:rows;
  for(const row of visibleRows)if(!all.has(rowKey(row)))all.set(rowKey(row),row);
 ...
 },[data?.proposed_rows,data?.original_rows,rows,proposalMode,filter,changes,columns,issues,allItems]);
 ...
 const addRow=()=>{if(!data||locked)return;setDraft(old=>[...old,newWorkspaceItem(data.artifact_type,old,columns)]);setManual(true);setNotice('');};
 const deleteRow=(itemId:string)=>{if(locked)return;setDraft(old=>old.filter(item=>String(item.id)!==itemId));setSelected(old=>old.filter(id=>id!==itemId));const chosen=changes.filter(change=>change.itemId===itemId);setDecisions(old=>({...old,...Object.fromEntries(chosen.map(change=>[change.key,'manual' as ReviewDecision]))}));setManual(true);setNotice('');};
 const addColumn=()=>{
  if(!data||locked)return;const field=columnField.trim();
  if(!field||field.startsWith('_')||columns.some(column=>column.field===field)){setError('请输入未使用且不以下划线开头的字段名。');return;}
  const definition={field,header:columnHeader.trim()||field};const next={...definition,editable:true};
  setData(old=>old?{...old,columns:[...old.columns,next]}:old);setDraft(old=>old.map(item=>({...item,[field]:''})));
  setColumnChanges(old=>({added:[...old.added.filter(column=>column.field!==field),definition],removed:old.removed.filter(value=>value!==field)}));
  setColumnField('');setColumnHeader('');setColumnEditor(false);setManual(true);setError('');
 };
 const removeColumn=(field:string)=>{
  if(locked)return;const wasAdded=columnChanges.added.some(column=>column.field===field);
  setData(old=>old?{...old,columns:old.columns.filter(column=>column.field!==field)}:old);
  setDraft(old=>old.map(item=>{const next={...item};delete next[field];return next;}));
  setColumnChanges(old=>({added:old.added.filter(column=>column.field!==field),removed:wasAdded?old.removed:[...new Set([...old.removed,field])]}));setManual(true);
 };
 const save=async()=>{
  ...
  try{
   ...
   if(saveReceipt.current?.fingerprint!==fingerprint)saveReceipt.current={fingerprint,id:globalThis.crypto?.randomUUID?.()??`workspace-grid-${Date.now()}-${Math.random()}`};
 ...
 const rejectProposal=async()=>{
  if(!data||!proposalMode||locked||dirty||editor||saving.current)return;
  saving.current=true;setBusy(true);setError('');
  try{
   const payload={items:cloneItems(data.original_items),layout,expected_revision:data.artifact_revision,profile_id:data.profile_id,profile_revision:data.profile_revision,run_id:data.run_id,proposal_id:data.proposal_id,prompt_id:data.prompt_id,proposal_decision:'reject'},fingerprint=JSON.stringify(payload);
   if(saveReceipt.current?.fingerprint!==fingerprint)saveReceipt.current={fingerprint,id:globalThis.crypto?.randomUUID?.()??`workspace-grid-${Date.now()}-${Math.random()}`};
   const result=await api<Json>(endpoint+'/save',{...payload,client_request_id:saveReceipt.current.id});
   latestDirty.current=false;setNotice(result.message??'已拒绝此建议。');await onSaved(result);
  }catch(cause){setError(errText(cause));}
  finally{saving.current=false;setBusy(false);}
 };
 ...
 const reset=()=>{if(data){projectionSerial.current++;setData(old=>old?{...old,columns:structuredClone(baselineColumns.current)}:old);setDraft(cloneItems(data.proposed_items));setRows(structuredClone(data.proposed_rows));setReportDraft(structuredClone(data.proposed_report??data.report??{}));setColumnChanges(data.column_changes??EMPTY_COLUMN_CHANGES);setDecisions({});setManual(false);setEditor(null);setActiveCell(null);setProjecting(false);setProjectionError(false);setNotice(proposalMode?'已撤销本地审阅，恢复 AI 建议预览。':'已撤销本地更改。');latestDirty.current=false;}};
 ...
 const artifactName=data?.artifact_type==='cases'?'测试用例':data?.artifact_type==='scenarios'?'测试场景':data?.artifact_type==='analysis'?'需求理解':'成果';
 return <div className="full-screen-workspace" role="dialog" aria-modal="true" aria-label="成果工作区" ref={container} tabIndex={-1} onKeyDown={onKeyDown}>
  <header className="full-review-toolbar"><div className="full-review-heading"><button aria-label="返回对话" onClick={close} disabled={busy||chatBusy}>← 返回对话</button><div><h2 id="full-review-title">{data?.title??artifactName} <span>· v{data?.artifact_revision??request.revision??'—'}</span></h2><p>{readOnly?'历史版本 · 只读查看':proposalMode?'AI 建议 · 逐项决定后保存':'手动编辑 · 保存为新版本'}</p></div></div>
   <div className="full-review-actions">{data?.artifact_type==='cases'&&<label>布局 <select aria-label="表格与 Excel 布局" value={layout} disabled={loading||busy||projecting} onChange={event=>setLayout(event.target.value)}><option value="case">每条用例一行</option><option value="step">每个步骤一行</option></select></label>}<button onClick={()=>void exportDraft()} disabled={!data||busy||projecting||projectionError||!!incoming||!!editor}>导出 Excel</button>{!readOnly&&<><button onClick={addRow}>添加{data?.artifact_type==='cases'?'用例':data?.artifact_type==='scenarios'?'场景':'条目'}</button>{data?.artifact_type==='cases'&&<button onClick={()=>setColumnEditor(value=>!value)}>管理自定义列</button>}{proposalMode&&<><button onClick={()=>decide(unresolved,'accept')} disabled={locked||!unresolved.length}>全部接受</button><button className="workspace-reject-proposal" onClick={()=>void rejectProposal()} disabled={locked||dirty||!!editor}>拒绝此建议</button>{(dirty||editor)&&<span className="workspace-reject-hint">请先保存或撤销本地更改，再拒绝整份建议。</span>}</>}<button className="primary" onClick={()=>void save()} disabled={locked||projecting||projectionError||(proposalMode&&!!unresolved.length)||!!editor||(!dirty&&!proposalMode)}>{busy?'正在保存…':proposalMode?'保存并完成确认':'保存更改'}</button><button aria-expanded={sidebar} onClick={()=>setSidebar(value=>!value)}>{sidebar?'收起助手':'打开助手'}</button></>}</div></header>
  {columnEditor&&!readOnly&&<div className="workspace-column-editor"><label>字段名<input aria-label="新列字段名" value={columnField} onChange={event=>setColumnField(event.target.value)}/></label><label>列名称<input aria-label="新列名称" value={columnHeader} onChange={event=>setColumnHeader(event.target.value)}/></label><button onClick={addColumn}>添加列</button>{columns.filter(isCustomColumn).map(column=><button key={column.field} aria-label={'删除列 '+column.header} onClick={()=>removeColumn(column.field)}>删除 {column.header}</button>)}</div>}
  <div className="full-review-filterbar"><div role="group" aria-label={'筛选'+artifactName}>{([['all','显示全部'],['changes','仅看 AI 修改'],['comments','仅看有批注'],['issues','仅看有问题']] as [Filter,string][]).map(([value,label])=><button key={value} aria-pressed={filter===value} onClick={()=>{setFilter(value);setVisible(100);}}>{label}</button>)}</div><span>{new Set(gridRows.map(row=>row.item_id)).size} 条{proposalMode?` · ${unresolved.length} 处待决定`:''}{selected.length?` · 已选 ${selected.length} 条`:''}{projecting?' · 正在更新表格…':''}</span>{dirty&&!readOnly&&<button disabled={busy} onClick={reset}>撤销本地更改</button>}</div>
  ...
  {data?.artifact_type==='analysis'&&Object.keys(reportDraft).length>0&&<details className="workspace-report-editor" open><summary>需求理解报告</summary>{!readOnly&&<><label>需求理解摘要<textarea aria-label="需求理解摘要" value={String(reportDraft.summary??'')} onChange={event=>{setReportDraft(old=>({...old,summary:event.target.value}));setManual(true);}}/></label>{Array.isArray(reportDraft.diagrams)&&reportDraft.diagrams.map((diagram:Json,index:number)=><label key={index}>{diagram.title??`业务图 ${index+1}`} · Mermaid<textarea aria-label={'图形源码 '+(index+1)} value={String(diagram.mermaid??'')} onChange={event=>{setReportDraft(old=>({...old,diagrams:old.diagrams.map((value:Json,i:number)=>i===index?{...value,mermaid:event.target.value}:value)}));setManual(true);}}/></label>)}</>}<div className="workspace-report-preview" aria-label="需求理解报告预览"><AnalysisReport report={reportDraft}/></div></details>}
  <div className={'full-review-body'+(sidebar&&!readOnly?' with-assistant':'')}>
   <main className="full-review-grid-region" aria-label={artifactName+'工作区'}>
    <div className="excel-grid-scroll"><table className="excel-grid-table" aria-label={artifactName+'工作表'}><thead><tr>{!readOnly&&<th className="review-selection-column" scope="col"><input type="checkbox" aria-label={'选择当前筛选全部'+artifactName} checked={gridRows.length>0&&gridRows.every(row=>selected.includes(row.item_id))} onChange={event=>setSelected(event.target.checked?[...new Set(gridRows.map(row=>row.item_id))]:[])} /></th>}{columns.map((column,index)=><th key={index} scope="col" className={canonicalField(column)==='id'?'review-id-column':''}>{column.header}</th>)}</tr></thead><tbody>{gridRows.slice(0,visible).map(row=>{
     ...
     return <tr key={key} data-item-id={row.item_id} className={(selected.includes(row.item_id)?'is-selected ':'')+(rowChange?'review-row-'+rowChange.operation:'')}>{!readOnly&&<td className="review-selection-column"><input type="checkbox" aria-label={`选择 ${row.item_id}${row.step_index===null?'':` 第 ${row.step_index+1} 步`}`} checked={selected.includes(row.item_id)} onChange={event=>setSelected(old=>event.target.checked?[...new Set([...old,row.item_id])]:old.filter(id=>id!==row.item_id))}/>{proposalMode&&rowChange&&<div className="review-row-decision"><span>{rowChange.operation==='add'?'AI 新增':'AI 删除'}</span>{decisions[rowChange.key]?<small>{decisions[rowChange.key]==='accept'?'已接受':'已拒绝'}</small>:<><button disabled={locked} aria-label={`接受 ${row.item_id} ${rowChange.operation==='add'?'新增':'删除'}`} onClick={()=>decide([rowChange],'accept')}>接受</button><button disabled={locked} aria-label={`拒绝 ${row.item_id} ${rowChange.operation==='add'?'新增':'删除'}`} onClick={()=>decide([rowChange],'reject')}>拒绝</button></>}</div>}{row.step_index===null&&draft.some(item=>String(item.id)===row.item_id)&&<button className="workspace-delete-row" aria-label={'删除条目 '+row.item_id} onClick={()=>deleteRow(row.item_id)}>删除</button>}</td>}
      {columns.map((column,index)=>{
       ...
       return <td key={index} className={(canonicalField(column)==='id'?'review-id-column ':'')+(changed?'review-changed-cell ':'')+(pending?'review-pending-cell':'')}>
        ...
        {activeCell===cellKey&&!locked&&<div className="review-cell-menu" role="group" aria-label={`${row.item_id} ${column.header} 操作`}>{proposalMode&&units.length>0&&!rowChange&&<><button onClick={()=>decide(units,'accept')}>接受修改</button><button onClick={()=>decide(units,'reject')}>拒绝修改</button></>}{canEdit?<button onClick={()=>openEditor(row.item_id,column)}>手动编辑</button>:<span>此列由模板计算或不可修改</span>}<button aria-label="关闭单元格操作" onClick={()=>setActiveCell(null)}>×</button></div>}
       </td>;
      })}</tr>;
    ...
    <footer className="full-review-grid-footer">表头、列顺序和单元格内容与当前 Excel 导出一致。{!readOnly&&'双击单元格或按 F2 编辑；步骤与预期保持配对。'}{proposalMode&&unresolved.length>0&&<strong>还有 {unresolved.length} 处建议需要接受或拒绝。</strong>}</footer>
   </main>
   {sidebar&&!readOnly&&<aside className="full-review-assistant" aria-label="成果助手"><header><strong>成果助手</strong><span>{selected.length?`针对已选 ${selected.length} 条`:`当前${artifactName}`}</span></header><div className="full-review-chat-log" ref={chatLog} role="log" aria-label="当前对话">{messages.filter(message=>typeof message.content==='string'&&message.content.trim()).slice(-12).map((message,index)=><div key={message.id??index} className={'review-chat-message '+(message.role==='user'?'user':'assistant')}><small>{message.role==='user'?'你':'AI'}</small><p>{message.content}</p></div>)}{!messages.length&&<p>可以提问，或让 AI 修改当前选中的条目。</p>}</div><form onSubmit={event=>{event.preventDefault();void send();}}><label htmlFor="review-chat-input">{selected.length?`让 AI 处理选中的 ${selected.length} 条`:'向 AI 提问或提出修改'}</label><textarea aria-label="成果对话输入" id="review-chat-input" value={chatDraft} onChange={event=>setChatDraft(event.target.value)} placeholder="例如：补充选中条目的边界情况" disabled={busy||chatBusy}/>{dirty&&<p className="review-draft-hint">本地更改已保留。保存或撤销后可继续让 AI 修改。</p>}<button className="primary" aria-label="发送成果消息" disabled={!chatDraft.trim()||locked||dirty||!!editor} type="submit">{chatBusy?'AI 正在处理…':'发送'}</button></form></aside>}
  </div>
