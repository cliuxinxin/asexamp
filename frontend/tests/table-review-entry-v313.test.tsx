...
const {TableReviewContext}=await import('../src/tableReviewContext');
...
const reviewData={artifact_id:'cases',title:artifact.title,artifact_revision:3,layout:'case',columns,original_items:artifact.items,proposed_items:[after,second],original_rows:rows(artifact.items),proposed_rows:rows([after,second]),issues:[{case_ids:['TC1'],field:'expected',title:'补充可验证预期'}],read_only:false,run_id:'run',proposal_id:'review_3',prompt_id:prompt.id};
...
test('historical ArtifactCard opens full-screen using the exact visible revision and read-only context',async()=>{
 ...
 render(<TableReviewContext.Provider value={{open:request=>opened.push(request)}}><ArtifactCard id="cases" snapshot={{...artifact,revision:2}} readOnly chatOnly simplified onTarget={()=>assert.fail('launch must not retarget chat')} onChanged={()=>assert.fail('launch must not mutate')}/></TableReviewContext.Provider>);
 fireEvent.click(screen.getByRole('button',{name:'全屏表格评审',exact:true}));
 ...
 await act(async()=>{});
 assert.ok(requests.every(path=>path.includes('/workspace?revision=2')));
});
...
test('pending review opens full-screen from its assistant card and returning preserves the main composer draft',async()=>{
 ...
 const entry=await within(card).findByRole('button',{name:'进入全屏表格评审'});
 ...
 const opened=new URL(request.reads.find(path=>path.includes('/table-review'))!,'http://localhost');
 assert.equal(opened.pathname,'/artifacts/cases/table-review');
...
test('side assistant uses the existing chat and selected cases without starting a workflow or consuming the main draft',async()=>{
 const request=appFixture();const {App}=await import('../src/App');render(<App/>);
 const entry=await screen.findByRole('button',{name:'进入全屏表格评审'});
 const input=screen.getByLabelText('聊天输入') as HTMLTextAreaElement;
 ...
 fireEvent.click(within(reviewer).getByRole('button',{name:/^发送(评审消息)?$/}));
