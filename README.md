# TCG Case Agent · 本地智能 Agent v2.1.0
...
新版默认使用智能 Agent：模型先制定计划，结合证据梳理业务图与测试策略；你确认方向后，模型继续生成场景和用例，再检查关联遗漏、定向补充并总结。分析摘要、计划和进度通过 SSE 展示，原始模型输出和请求内容按需展开。

- **AI 推荐 / 快速设计 / 标准设计 / 深度设计**：按业务风险选择测试方法，理由可见。这些是本产品的设计深度，不是专业认证等级，也不是固定用例数量。
- **先确认业务图**：本机 Mermaid 渲染，可放大、查看源码和下载 SVG；用自然语言反馈即可重新分析。统计关联覆盖，不代表实际执行过测试。
- **执行中补充要求**：指令先持久化，在安全边界重新规划；过期范围的输出不会覆盖新要求。
- **多轮记忆**：确认的范围、用户补充、未决问题和证据随会话保存；旧聊天、版本和检查点继续使用。
- **成果工作区**：聊天保留摘要，点击成果卡片在右侧查看、编辑、核查依据和导出。项目偏好放在设置中。
- **可操作的错误说明**：显示问题、已保留内容和建议。认证错误不会自动反复请求；格式修复和规划循环有明确预算。
- **自定义 Header**：支持 `.env`、加密的本地设置和仅 Header 鉴权；连接测试实际校验模型的 JSON 回复。只接受文本块数组的内网网关可启用 [minimal 兼容模式](docs/INTRANET_GATEWAY.md)。
- **按需读取文档**：规划和总结使用目录/已接受结论；Agent 可本地搜索及读取相关段落。全文分析和生成分批执行，成功分析批次跨重试与重启复用，格式错误局部修正。详见[文档工作区与执行方式](docs/AGENT_DOCUMENT_WORKSPACE.md)。

启动仍使用 `python3 start.py`。升级前停止旧服务、备份整个数据目录，再用 `--data-dir` 指向原目录。新聊天请求走 v2 Agent；旧任务仍由原图恢复，不重写旧检查点。

使用与技术说明见 [智能 Agent 指南](docs/INTELLIGENT_AGENT.md) 和 [新增接口](docs/AGENT_API.md)。
...
3. 查看计划、分析摘要、依据和业务图。关键业务歧义会先提问。默认在生成前等待你确认测试方向；可在方案中关闭一般策略确认，但关键歧义仍需回答。
4. 对业务图有意见，直接在聊天里补充。新图确认后继续生成；生成中也能补充本轮要求。
...
backend/tcg/agent.py      v2 智能图、规划、确认、补充指令、会话记忆
...
OpenAI 兼容服务须支持 Chat Completions 的流式 JSON 响应，Ollama 使用流式消息接口。连接测试使用与生成相同的流式 JSON 请求路径，并校验返回 `{ "ok": true }`；它验证连接、鉴权、流式接口和基础 JSON 输出，不代表模型一定能完成复杂业务设计。v2.0.6 的单次请求时限统一为 60 分钟；有限重试规则不变，每次重试也最多等待 60 分钟。
