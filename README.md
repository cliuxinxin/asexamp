...
新版默认按小工作项推进：读取一组需求、保存业务理解与图、设计场景、生成用例，全部范围处理后进入一次评审阶段。已知下一步由程序调度；模型可以主动请求搜索、读取原文或已保存规则，不再每一步都重写整套计划。

- **Auto 自动推进**：业务问题和未确认假设保留在成果中；需要确认方向时，展开“本次方案”勾选确认。确实没有业务输入时才要求补充资料。
- **最小上下文**：分析只发当前片段，生成只发相关规则与场景，评审只发当前组。需要原文时通过只读工具补查；超长段落保留引用与偏移，不截断后宣称全文完成。
- **尽早看到草稿**：SSE 展示当前动作、来源、进度、阶段结论和失败原因。阶段草稿始终只读，最终成果发布后才提供编辑、版本与导出。
- **局部恢复与修改**：每组通过校验立即保存；失败重试保留已完成工作。执行中补充要求会检查影响，只更新受影响的已完成组；过期回复不能写入。
- **业务图和覆盖核查**：Mermaid 本机渲染，显示需求、分支、场景、用例关联。生成用例后固定一次 AI 评审阶段，可分组；不按评分反复整批生成。关联覆盖不等于已执行测试或证明业务正确。
- **简洁聊天操作**：保留 AI 推荐、快速、标准、深度设计；可暂停、继续、补充要求，并按需点开全部工作、AI 请求和输出。
- **现有模型配置**：沿用 `.env`、自定义 Header、仅 Header 鉴权、[内网 minimal 模式](docs/INTRANET_GATEWAY.md)及 60 分钟请求超时。上游非流式时显示真实等待，正文返回后展示，不模拟逐字输出。

启动仍使用 `python3 start.py`。本次只支持新建的增量 Agent 任务，不迁移 v2 未完成任务或检查点；既有来源、配置和已发布成果保留。升级前停止旧进程，再启动新版并新建任务。

详细实现、流程图和验证边界见 [增量 Agent 指南](docs/INCREMENTAL_AGENT.md)，接口见 [Agent API](docs/AGENT_API.md)。旧版指南标记为历史设计，不能用来判断新版行为。
...
配置读取规则：系统环境变量优先于文件；文件选择顺序为显式 `--env-file` / `TCG_ENV_FILE`，否则选数据目录 `.env`，不存在才选项目根目录 `.env`。只读一个文件，不合并多个文件。已选文件中的字段覆盖 `settings.json`；未提供字段沿用已有配置或默认值。不存在的显式文件、格式错误、错误 provider/URL/超时会明确报错。文件按 UTF-8 读取，支持 `export NAME=value`、单/双引号和注释；值含空格或 `#` 时加引号，只支持单行，不执行命令或展开变量。另外支持 `TCG_MODEL_AUTH_MODE` 和 `TCG_MODEL_HEADERS_JSON`，完整示例见 `.env.example`。
...
普通模式使用 Chat Completions 和 `response_format={"type":"json_object"}`；只接受 `model/messages` 的网关使用 minimal 模式，详见内网接入指南。模型密钥在本机加密保存，设置接口不返回明文。更换服务地址后，原密钥不会自动用于新服务。普通本地无密钥服务可留空。

本项目不会自动创建付费账户、获取密钥或下载大型模型。未配置模型时会明确提示，不会用预置结果冒充生成结果。

## 使用流程

1. 开始对话，上传需求或直接描述业务；已上传的来源可以在“需求来源”核查。附件默认作为主要需求，其他用途放在“本次方案 → 指定任务与资料用途”。
2. 直接发送任务；默认由 AI 推荐深度。需要调整时展开“本次方案”，选择快速、标准或深度设计。
3. 查看当前工作、分析结论、依据和业务图。默认 Auto 连续推进并记录未决问题；勾选确认方向后，会在生成前等待你确认。
4. 对业务图有意见，直接在聊天里补充。系统重新处理受影响的组；生成中也能补充本轮要求，或暂停、继续。
5. 点击成果卡片打开工作区，查看需求 → 分支 → 场景 → 用例关联、编辑选中条目、查看版本及下载 Excel。修改已有成果须等当前任务结束。
6. 常用规则在“模型与设置 → 项目偏好”维护；点击“使用此项目偏好”供后续任务使用。

分析需求、生成场景、生成用例、评审用例、证据问答、学习模板和选中修改均保留。未指定目标时由模型路由。旧 API 不传 `experience` 时仍采用 legacy 图，兼容原有 Auto / HITP 和场景确认行为。

## 技术结构

```text
backend/tcg/main.py       FastAPI 接口、应用生命周期、前端静态文件
backend/tcg/graph.py      原版图、共享执行器、SSE、检查点恢复
backend/tcg/incremental_agent.py  v3 增量图、工作项、按需工具、局部修复和发布
backend/tcg/incremental_workspace.py  小组、持久结果、任务上下文预算
backend/tcg/incremental_tasks.py  各能力的 JSON 返回契约
backend/tcg/agent.py      历史 v2 图及共享反馈、诊断辅助函数
backend/tcg/agent_contracts.py  业务图、证据、深度、设计覆盖契约
...
实际模型输出是按任务契约生成的 JSON，生成途中可能不完整，展开“查看调用详情”后卡片会标明“尚未校验”。最终用例仍需通过字段、证据及版本检查后才能保存和发布。时间线展示模型响应正文及执行事件，不读取独立的 reasoning/thinking 字段。系统提示词和任务输入可按需点开核查，请求头值在请求记录中统一脱敏。
...
普通 OpenAI 兼容模式使用 Chat Completions 流式 JSON 响应，Ollama 使用流式消息接口；minimal 模式允许上游返回完整非流式 JSON。连接测试使用与生成相同的流式 JSON 请求路径，并校验返回 `{ "ok": true }`；它验证连接、鉴权、流式接口和基础 JSON 输出，不代表模型一定能完成复杂业务设计。v2.0.6 的单次请求时限统一为 60 分钟；有限重试规则不变，每次重试也最多等待 60 分钟。
...
- 旧图保留分批分析；新 Agent 的场景和用例支持显式分页，分析使用当前完整证据。单个模型上下文超过 50 万字符会明确失败，不静默截断；超大需求需拆分范围。已有确定性的需求/分支关联检查，尚未实现完整 Token 预算规划和跨批语义去重。
- 复杂需求的业务模型由 AI 按契约生成，后端转换为 Mermaid，前端本地绘图。支持放大、源码、下载和自然语言修改，尚无拖拽式图形编辑器。
