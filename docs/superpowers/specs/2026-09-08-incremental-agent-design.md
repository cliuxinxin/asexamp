...
日期：2026-09-08。状态：按已批准方案实施。本文保留设计依据与目标；实际实现、验证结果和边界见 [增量 Agent 指南](../../INCREMENTAL_AGENT.md)。真实内网模型性能尚未验证。
...
后续用户明确取消旧任务迁移要求。新任务使用 graph_version=3；v2 未完成任务提示新建任务，不把旧状态交给新图解释。模型配置、来源和已发布成果保留。
...
运行代码、增量存储和 UI 已实施；受控验收覆盖调用数、按需读取、局部恢复、指令变更与发布约束。真实内网模型的速度和质量对比仍待用户本地验证。旧任务迁移已按用户要求取消。具体实现偏差与限制以增量 Agent 指南为准。
