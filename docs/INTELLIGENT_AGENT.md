...
需求澄清时可以点击“按当前信息继续”，也可以直接说“不要对了，就这样吧”或“按现有信息继续”。系统保留当前分析，进入后续设计；未决问题和假设会在成果与总结中明确列出，不会被当成已确认的规则。按钮不要求逐一回答所有问题，也不会清除已填写的补充内容。

如果你同时补充了规则并要求继续，例如“锁定阈值是4次，其他问题先保留，直接生成”，系统先应用新规则，再继续。后续再次修改业务范围会重新检查策略。尚未提供任何业务资料的任务仍需先提供要测试的内容；此时没有可沿用的分析。
...
测试连接使用当前配置的实际模型传输方式，检查连接、鉴权和最小结构化响应。任务超时仍为每次模型调用 60 分钟；无法改变提供方自己的网关超时。

若内网网关要求 `messages[].content` 为文本块数组且仅接受 `model/messages`，请配置 `TCG_MODEL_REQUEST_MODE=minimal`，详见[内网网关接入说明](INTRANET_GATEWAY.md)。这个模式的上游返回普通 JSON；前端继续通过 SSE 展示节点进度和返回结果。
