...
v3.0.3 adds env-only Azure selection: nonempty `AZURE_OPENAI_API_KEY`, `AZURE_OPENAI_ENDPOINT`, and `AZURE_API_VERSION` take priority over all legacy provider/address/auth settings. Any nonempty Azure field enables validation; incomplete configuration is rejected. `AZURE_OPENAI_DEPLOYMENT` falls back to the effective legacy model name; a deployment is required. Same-name process fields override the chosen env file. All Azure fields empty/absent retain the old behavior. `/api/settings` returns `provider: "azure"`, the deployment in `model`, and additive `api_version`; this configuration is environment-managed. Manual SettingsRequest remains openai/ollama only; `/api/settings/test` uses the active environment configuration. Azure uses the dated deployment Chat Completions route and only `api-key` authentication, with no legacy custom headers. Saved legacy settings are not rewritten. Native `bind_tools` remains available for both chat and pipeline.

v3.0.4 adds optional `reply_kind: "question" | "clarification" | "confirm"` on a chat turn. UI quick replies set this explicit user scope and the existing `reply_to`, and send natural-language content through the same agent endpoint. Question scope exposes only read tools; clarification adds only answer submission; confirm adds only current pipeline stage approval. Ordinary typed chat omits the field and retains the full registry. `resume` requires a clarify/clarified action plus explicit answers at clarification gates, and refuses clarification actions at review gates. Native report schemas allow extensions, but typed core fields and source validation remain enforced; runtime metadata is filtered before saving. Canonical `report.questions`, including an empty array, overrides legacy aliases. After clarification, pending questions are computed from actual matched answers, not inferred from model omissions or repeated questions.
