...
def response_excerpt(raw,issue):
    ...
    parts=['原始返回较长，下列为节选；不是模型返回被截断。','返回开头：',code(clip(raw,1200))]
...
def failed_step_report(store,diagnostics,run_id,call_id=None):
    ...
    parts=['# TCG 失败步骤日志',f'任务：{run_id}',f'当前状态：{run["status"]}；步骤：{node or run.get("failed_stage") or run["stage"]}',
        '错误：'+clip(run.get('error') or '当前无终止错误；以下为所选调用或最近步骤。',1800),
    ...
    for i,call in enumerate(chosen,1):
        ...
        own=[e for e in events if e.get('call_id')==cid]
        ...
        specific=[{k:e[k] for k in ('event','parse_error','validation_error','validation_errors','errors','error_types','http_status','stack') if k in e} for e in own if e.get('event') in ('model.invalid_json','model.error','node.error','batch.validation_failed','model.transport_error')]
