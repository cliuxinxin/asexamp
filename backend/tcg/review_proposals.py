"""Version-bound review proposals, separate from accepted case artifacts."""
import copy
...
def save_review_proposal(store, run, cases, rows, report, guard, sources, roles, feedback=''):
    ...
    with store.transaction():
        ...
        try:
            return store.get('review_proposal', proposal_id)
        except DomainError as exc:
            if exc.status != 404:
                raise
        return store.put('review_proposal', {'id': proposal_id, 'run_id': run['id'],
            ...
            'changes': case_changes(cases['items'], rows), 'feedback': feedback,
...
def read_review_proposal(store, run_id, proposal_id):
    ...
    proposal = store.get('review_proposal', proposal_id)
