...
def create_app(data_dir: Path | str | None = None, model_gateway=None):
    ...
    app = FastAPI(title='TCG Case Agent Local', version='3.0.15', lifespan=lifespan)
    ...
    @app.get('/api/health')
    def health():
        return {'status': 'ok', 'version': '3.0.15', 'storage': 'local', 'model_configured': configured()}
    ...
    def run_diagnostics(run_id: str, download: bool = False):
        ...
        payload = {
            'version': '3.0.15', 'run_id': run_id, 'chat_id': run['chat_id'],
